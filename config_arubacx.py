...
# scripts/config_arubacx.py
#
# Import/Export script for arubacx switch simulator.
#
# @author Andrea Dainese <andrea.dainese@gmail.com>
# @author Alain Degreffe <eczema@ecze.com>
# @author Stephane Duchesne <drannor.dosalith@gmail.com>
# @copyright 2014-2016 Andrea Dainese
# @copyright 2017-2018 Alain Degreffe
# @copyright 2022-2023 Stephane Duchesne
...
def node_login(handler):
    ...
    if i == 0:
        ...
        try:
            n = handler.expect('new password', timeout = expctimeout)
            #print('DEBUG: The value of newpass is: ', n)
            if n == 0 :
                handler.sendline(password)
                try:
                    handler.expect('Confirm', timeout = expctimeout)
                except:
                    print('ERROR: error waiting for "Confirm new password:" prompt.')
                    node_quit(handler)
                    return False
                handler.sendline(password)
            else :
...
def config_get(handler):
    ...
    config = re.sub('.*Current configuration:\n', '', config, flags=re.DOTALL)          # Header
...
def config_put(handler, config):
    ...
    # Save
    handler.sendline('write memory')
    try:
        handler.expect('\[Success\]', timeout = expctimeout)
    except:
